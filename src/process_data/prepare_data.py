...
from process_data.constants import NEW_COL_NAMES
...
def process_csv(csv_file_path: str, cols_to_retain: list[str], new_column_names: list[str]) -> None:
    """
    ...
    df.rename(columns=dict(zip(cols_to_retain, new_column_names)), inplace=True)
