...
def add_target_column(
        ...
        target_col_name: str = TARGET_COL_NAME) -> pd.DataFrame:
    """
    ...
    if any(s in df_cols for s in expected_cols_tuple):
        if expected_cols_tuple[0] in df_cols:
            # Rename col for consistency wrt other 'status' cols in the other
            # dfs
            updated_df.rename(
                columns={
                    expected_cols_tuple[0]: target_col_name},
                inplace=True)
            ...
            updated_df.rename(
                columns={
                    expected_cols_tuple[1]: target_col_name},
                inplace=True)
    else:
        # Creating a new column called 'status' with 1s because all rows
        # pertain to PD subjects (for the data of Max Little in 2009)
...
def save_processed_df(
    ...
    processed_df.to_csv(
        ...
        f"{initial_file_path.split('.')[0]}{processed_suffix}{CSV_FMT_STR}",
        index=False)
